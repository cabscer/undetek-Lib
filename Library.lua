...
local Library = {
    ...
    NumberColor = Color3.fromRGB(100, 150, 255);
    ButtonHighlight = Color3.fromRGB(255, 140, 0);
    MainColor = Color3.fromRGB(15, 15, 15);
    BackgroundColor = Color3.fromRGB(10, 10, 10);
    AccentColor = Color3.fromRGB(255, 105, 180); -- Pink/Magenta for UNDETEK II
...
do
    ...
    function Funcs:AddButton(...)
        ...
        -- UNDETEK II Style: Orange highlighted row
        local function CreateBaseButton(Button)
            ...
            Library:AddToRegistry(Outer, {
                BackgroundColor3 = 'ButtonHighlight';
            });
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        -- UNDETEK II Style: Row with left label and right On/Off value
        ...
        -- Value label showing "On" (green) or "Off" (red)
        ...
        -- Container for addons (keypicker, colorpicker)
        ...
        return Toggle;
    end;function Funcs:AddSlider(Idx, Info)
        ...
        -- UNDETEK II Style: Row with left label and right value in blue
        ...
        -- Invisible slider interaction area
        local SliderRegion = Library:Create('Frame', {
            BackgroundTransparency = 1;
            Size = UDim2.new(1, 0, 1, 0);
            ZIndex = 8;
            Parent = SliderRow;
        ...
        if type(Info.Tooltip) == 'string' then
            Library:AddToolTip(Info.Tooltip, SliderRegion)
        end

        function Slider:UpdateColors()
            -- No fill to update in UNDETEK style
        end;
        ...
        SliderRegion.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                local startX = Mouse.X;
                local startVal = Slider.Value;
                local sensitivity = (Slider.Max - Slider.Min) / 200;

                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    local deltaX = Mouse.X - startX;
                    local newVal = math.clamp(startVal + (deltaX * sensitivity), Slider.Min, Slider.Max);
                    newVal = Round(newVal);
        ...
        return Slider;
    end;function Funcs:AddDropdown(Idx, Info)
...
function Library:CreateWindow(...)
    ...
    function Window:AddTab(Name)
        ...
        -- No corner radius for UNDETEK II sharp style
